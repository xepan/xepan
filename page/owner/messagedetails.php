...
class page_owner_messagedetails extends page_base_owner{
	function init(){
		...
		$msg_detail->add('View')->setHTML('Name - '.$msg['name']);
		$msg_detail->add('View')->setHTML('From - '.$msg['sender_signature']);
		$msg_detail->add('View')->setHTML('Name Space - '.$msg['sender_namespace']);
		$msg_detail->add('View')->setHTML('Message - '.'<br>'.$msg['message']);
