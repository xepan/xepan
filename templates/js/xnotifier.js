...
jQuery.widget("ui.xnotifier",{
	...
	render: function (){
		...
		$(self.options.activities).each(function(index,element){
			activity_box = $('<div class="atk-box atk-row atk-size-micro atk-padding-small"></div>').appendTo(self.options.activity_area);
			...
			document_root = $('<small class="atk-col-12 atk-hr-small text-center" style="cursor:pointer;">'+element.related_root_document_name+' :: '+element.related_document+'</small>').appendTo(activity_box);
			//subject
			subject = $('<div class="atk-col-12 atk-effect-info atk-label atk-size-reset" style="cursor:pointer;">'+element.subject+'</div><div class="atk-hr-large"></div>').appendTo(activity_box);
			//Action
			// str = str+'<div class="atk-col-12 text-center">Action</div>';
			//Action from
			str = '<div class="atk-col-5 icon-user">'+element.action_from+'</div>';
