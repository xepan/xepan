...
class Filter_Campaign extends \Filter_Base
{
    ...
    function init()
    {
        ...
        $this->type_field = $this->addField('Dropdown', 'category', '')->setEmptyText('All Campaign Category')->setNoSave()->setModel('xMarketingCampaign/Model_CampaignCategory');
    ...
    function postInit()
    {
        ...
        $v = trim($this->get('q'));
        $camcat= $this->get('category');
        // $status = $this->get('status');

        if(!$v AND !$camcat /*AND !$status*/) {
            return;
        }
