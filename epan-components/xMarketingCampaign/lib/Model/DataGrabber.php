...
class Model_DataGrabber extends \Model_Table{
	...
	function init(){
		...
		$f=$this->addField('site_url')->hint('Website URL, from where to grab data')->mandatory(true)->group('a~5')->sortable(true);
		...
		$this->addExpression('last_run_time')->set('last_run_at')->sortable(true);
