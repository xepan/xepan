...
class View_MemberTemplate extends \View {
	...
	function init(){
		...
		$my_designs_model = $this->add('xShop/Model_Item');
		$my_designs_model->addCondition(
						$my_designs_model
						->dsql()
						->orExpr()
						->where('to_customer_id',$designer->id)
		...
		if(!$crud->isEditing()){
			...
			// if(!$member['is_organization'] ){
				...
				$g->addMethod('format_edit_template',function($g,$f)use($designer,$designer_page){
					// echo $this->api->url();
					if($g->model->get('designer_id') == $designer->id){
						$img = '<img style="box-shadow:0 3px 5px rgba(0, 0, 0, 0.2);" class="atk-align-center" src="index.php?page=xShop_page_designer_thumbnail&xsnb_design_item_id='.$g->model['id'].'" width="100px;"/>';
						$g->current_row_html[$f]=$img.'</br>'.'<a class="icon-pencil atk-margin-small" target="_blank" href='.$g->api->url('index',array('subpage'=>$designer_page,'xsnb_design_item_id'=>$g->model['id'],'xsnb_design_template'=>'true')).'>Edit Template</a>';
					}
					else
			...
			//New Design
			$g->addColumn('new_design');
			// $subpage = $_GET['designer_page'];//$this->html_attributes['xsnb-desinger-page'];
			$g->addMethod('format_new_design',function($g,$f)use($designer,$designer_page,$my_designs_model){
				$g->current_row_html[$f]= '<a target="_blank" class="atk-margin-small" href='.$g->api->url('index',array('subpage'=>$designer_page,'xsnb_design_item_id'=>$g->model['id'])).'>New Design</a>';
			});
			$g->addFormatter('new_design','new_design');
