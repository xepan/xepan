...
class page_xShop_page_filterlayoutcss extends Page {
	function init(){
		parent::init();

		$css_name = 'filter.css';
		$css_file = getcwd().DS.'epans'.DS.$this->api->current_website['name'].DS.'xshopfilter.css';
		...
		$form = $this->add('Form');
		$form->addField('text','css')->addClass('xfilter-customcss')->set($css_content);
		...
		if($form->isSubmitted()){
			...
			$form->js(
				null,
				$form->js()->_selector('#xshop-filter-customcss-link')->attr('href',$user_css."?".rand(1000,9999))
